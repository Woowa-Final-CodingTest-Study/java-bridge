...
public class BridgeGame {
    ...
    public List<BridgeEnum> currentBridge() {
        List<BridgeEnum> bridgeEnums = new ArrayList<>();
        AtomicInteger startBridge = new AtomicInteger();

        for (String location : userAnswer) {
            BridgeEnum bridgeEnum = BridgeEnum.createBridgeStringType(location,
                    bridge.get(startBridge.getAndIncrement()).equals(location));
            bridgeEnums.add(bridgeEnum);
        }
